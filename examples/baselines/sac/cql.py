...
class Args:
    ...
    log_freq: int = 1000
    eval_freq: int = 1000  # evaluation frequency in training iterations
    save_model_dir: Optional[str] = "runs"

    # CQL hyperparams
    cql_variant: str = "R"
    offline_training_steps: int = 100000  # run longer
    ...
    # Additional hyperparameter: gradient clipping (if needed)
    grad_clip: float = 1.0
    ...
    # New evaluation parameters (adapted from SAC template)
    num_eval_steps: int = 50
    eval_partial_reset: bool = False

    # Demo path (offline dataset)
    demo_path: Optional[str] = None

    # Where to save the offline buffer
...
# Updated SoftQNetwork with an extra layer in the network
...
# Updated Actor network with an extra layer in the backbone
...
# Adapted evaluation function (following SAC template)
...
def offline_training_phase(
    ...
    for _ in pbar:
        ...
        if args.cql_variant.upper() == "H":
            ...
            q1_pol_lse = torch.logsumexp(q1_pol / temp, dim=1) * temp
            q2_pol_lse = torch.logsumexp(q2_pol / temp, dim=1) * temp
        ...
        if (global_update % args.log_freq == 0) and (logger is not None):
            ...
            # Additional logging of raw Q values and bellman errors:
            logger.add_scalar("raw/q1_pred_mean", q1_pred.mean().item(), global_update)
            logger.add_scalar("raw/q1_pred_std", q1_pred.std().item(), global_update)
            logger.add_scalar("raw/q1_pred_min", q1_pred.min().item(), global_update)
            logger.add_scalar("raw/q1_pred_max", q1_pred.max().item(), global_update)
            logger.add_scalar("raw/q2_pred_mean", q2_pred.mean().item(), global_update)
            logger.add_scalar("raw/q2_pred_std", q2_pred.std().item(), global_update)
            logger.add_scalar("raw/q2_pred_min", q2_pred.min().item(), global_update)
            logger.add_scalar("raw/q2_pred_max", q2_pred.max().item(), global_update)
            logger.add_scalar("raw/q_target_mean", next_q.mean().item(), global_update)
            logger.add_scalar("raw/q_target_std", next_q.std().item(), global_update)
            logger.add_scalar("raw/q_target_min", next_q.min().item(), global_update)
            logger.add_scalar("raw/q_target_max", next_q.max().item(), global_update)
            bellman_error_q1 = torch.abs(q1_pred - next_q)
            bellman_error_q2 = torch.abs(q2_pred - next_q)
            logger.add_scalar("bellman/q1_error_mean", bellman_error_q1.mean().item(), global_update)
            logger.add_scalar("bellman/q1_error_std", bellman_error_q1.std().item(), global_update)
            logger.add_scalar("bellman/q2_error_mean", bellman_error_q2.mean().item(), global_update)
            logger.add_scalar("bellman/q2_error_std", bellman_error_q2.std().item(), global_update)
            # Policy statistics:
            logger.add_scalar("policy/log_pi_mean", log_pi.mean().item(), global_update)
            logger.add_scalar("policy/log_pi_std", log_pi.std().item(), global_update)
            logger.add_scalar("policy/log_pi_min", log_pi.min().item(), global_update)
            logger.add_scalar("policy/log_pi_max", log_pi.max().item(), global_update)
            policy_mean, _ = actor(data.obs)
            logger.add_scalar("policy/mu_mean", policy_mean.mean().item(), global_update)
            logger.add_scalar("policy/mu_std", policy_mean.std().item(), global_update)
        pbar.set_postfix({
