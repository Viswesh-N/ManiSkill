...
# Available colored table PickCube variants
COLORED_TABLE_VARIANTS = [
    'PickCube-v1',  # Original
    'PickCubeBlueTable-v1',
    'PickCubeYellowTable-v1', 
    'PickCubeGreenTable-v1',
    'PickCubeRedTable-v1',
    'PickCubeBlackTable-v1',
    'PickCubeGrayTable-v1'
]
...
def select_eval_env_id(base_env_id):
    """Select a random colored table variant for evaluation if using PickCube"""
    if base_env_id == 'PickCube-v1':
        selected = random.choice(COLORED_TABLE_VARIANTS)
        print(f"Using eval environment: {selected}")
        return selected
    return base_env_id

def make_envs(cfg, num_envs, record_video_path, is_eval, logger):
	"""
	Make ManiSkill3 environment with colored table variants for evaluation.
	"""
	record_episode_kwargs = dict(save_video=True, save_trajectory=False, record_single=True, info_on_video=False)

	# For evaluation, randomly select colored table variant
	env_id = cfg.env_id
	if is_eval:
		env_id = select_eval_env_id(cfg.env_id)
